...
// ── ITEM DATA ────────────────────────────────────────────────────────────────
const ALL_ITEMS = [
  { id: "28073", name: "Rotisserie Rib Glaze 4/1G", category: "Dry Ingredients", barcode: "096619280735" },
  ...
  { id: "42897", name: "Dijon Mayonnaise", category: "Refrigerated Ingredients", barcode: "400000428970" }, // 4 00000 42897 0
  ...
  { id: "1613423", name: "Cheddar Cheese Slcd 6/1.5", category: "Refrigerated Ingredients", barcode: "071905944027" },
  ...
  { id: "1833268", name: "Yakisoba 4 oz Pouches (Rikki)", category: "Refrigerated Ingredients", barcode: "786764881108" },
  ...
  { id: "1839815", name: "Soy Sauce", category: "D63 Sushi/Sashimi", barcode: "073890559163" },
...
// Exactly matches the barcodes printed on the Costco supply worksheet
// ── UPC-A BARCODE ENCODER ─────────────────────────────────────────────────────
// All barcodes in the PDF are 12-digit UPC-A. We encode them exactly as-is —
// the stored 12 digits are the complete barcode including the check digit.
// Structure: start(101) + 6×L-digit(7) + centre(01010) + 6×R-digit(7) + end(101) = 95 modules
const UPCA_L = ["0001101","0011001","0010011","0111101","0100011","0110001","0101111","0111011","0110111","0001011"];
const UPCA_R = ["1110010","1100110","1101100","1000010","1011100","1001110","1010000","1000100","1001000","1110100"];
...
function encodeUPCA(raw) {
  ...
  for (let i = 0; i < 6; i++) bits += UPCA_L[d[i]];
  bits += "01010";  // centre guard
  for (let i = 6; i < 12; i++) bits += UPCA_R[d[i]];
  bits += "101";    // end guard  — total always 95 modules

  // Convert bit string → [{w, black}] run-length segments
  const bars = [];
  let i = 0;
  while (i < bits.length) {
    const ch = bits[i]; let w = 0;
...
function Barcode({ value, label }) {
  const result = useMemo(() => encodeUPCA(value), [value]);
